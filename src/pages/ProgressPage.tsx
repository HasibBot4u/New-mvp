...
import { BookOpen, Clock, Flame, PlayCircle, TrendingUp, CheckCircle } from 'lucide-react';
...
export function ProgressPage() {
  ...
  const { getStats, isCompleted, updateTrigger } = useVideoProgress();
      ...
      for (const cycle of subject.cycles) {
        ...
        for (const chapter of cycle.chapters) {
          for (const video of chapter.videos) {
            ...
            const completedStatus = isCompleted(video.id);
            if (completedStatus) {
              completed++;
              subjCompleted++;
              cycleCompleted++;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-5">
                      ...
                      <p className="text-xs text-gray-500 mb-4 truncate">{item.chapter.name}</p>
                    ...
                    {subject.cycles.map((cycle: any) => (
                      <div key={cycle.id} className="flex flex-col">
                        <div className="flex justify-between items-center mb-1 text-sm">
                          ...
                          <span className="text-gray-500 text-xs">{cycle.completed}/{cycle.total} videos</span>
                        </div>
              ...
              {recentlyCompleted.map((item, index) => (
                ...
                >
                  <div className="flex items-start">
                    ...
                    <div>
                      <h4 className="font-bold text-gray-900 text-sm mb-1">{item.video.title}</h4>
                      <p className="text-xs text-gray-500">
                        {item.subject.name} • {item.chapter.name}
                      </p>
                    </div>
                  </div>
                </Link>
